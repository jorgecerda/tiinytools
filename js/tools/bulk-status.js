...
export default {
    ...
    bindEvents() {
        ...
        optionsToggleBtn.addEventListener('click', () => {
            ...
            optionsToggleBtn.querySelector('svg').style.transform = isHidden ? 'rotate(180deg)' : 'rotate(0deg)';
        });
        ...
        resultsFilter.addEventListener('input', (e) => this.filterResultsTable(e.target.value));
    ...
    async startChecking() {
        ...
        if (rawUrls.length > 100) {
            alert('To preserve performance, checking is limited to a maximum of 100 URLs at a time.');
            return;
        }
        ...
        if (isCancelled) {
            document.getElementById('progressStatusText').textContent = 'Check Canceled';
        } else {
            document.getElementById('progressStatusText').textContent = 'All Checks Completed!';
        }
    },
    ...
    cancelChecking() {
        ...
        for (let i = 0; i < totalCount; i++) {
            if (!resultsData[i]) {
                ...
                if (row) {
                    ...
                    row.querySelector('.status-badge').style = 'background-color:rgba(0,0,0,0.1); color:var(--text-muted); border:1px solid var(--border-color);';
        ...
        document.getElementById('progressStatusText').textContent = 'Checking Cancelled';
    },
    ...
    updateStatsCounters(data) {
        ...
        if (finalHop.error) {
            stats.failed++;
        } else if (finalHop.status >= 200 && finalHop.status < 300) {
            stats.success++;
        } else if (finalHop.status >= 300 && finalHop.status < 400) {
            stats.redirect++;
        } else if (finalHop.status >= 400 && finalHop.status < 500) {
            stats.clientErr++;
        } else if (finalHop.status >= 500) {
            stats.serverErr++;
        }
    ...
    updateRowUI(i, data, statusTextOverride) {
        ...
        if (statusTextOverride) {
            ...
            row.querySelector('.status-badge').className = 'status-badge';
            row.querySelector('.status-badge').style = 'background-color:rgba(99,102,241,0.05); color:var(--primary); border:1px solid rgba(99,102,241,0.15);';
            return;
        }

        // Renders complete final response
        const finalHop = data.chain[data.chain.length - 1];
        const initialHop = data.chain[0];
        
        let statusBadgeClass = 'status-badge ';
        let statusLabel = finalHop.status ? `${finalHop.status} ${finalHop.statusText}` : finalHop.statusText;
        ...
        // Content size
        const sizeHeader = finalHop.headers ? finalHop.headers['content-length'] : null;
    ...
    toggleRowDetails(i) {
        ...
        data.chain.forEach((hop, idx) => {
            const badgeClass = hop.error ? 'badge-err' : (hop.status >= 200 && hop.status < 300 ? 'badge-2xx' : (hop.status >= 300 && hop.status < 400 ? 'badge-3xx' : (hop.status >= 400 && hop.status < 500 ? 'badge-4xx' : 'badge-5xx')));
            chainListHtml += `
        ...
        // Format headers list
        let headersHtml = '';
        if (finalHop.headers && Object.keys(finalHop.headers).length > 0) {
            Object.entries(finalHop.headers).forEach(([name, val]) => {
                headersHtml += `
            ...
            headersHtml = '<div style="color:var(--text-muted);">No headers available.</div>';
        }
    ...
    exportCsv() {
        ...
        csvContent += 'Index,Requested URL,Final URL,Hops,Final Status Code,Status Text,Total Response Time (ms),Size (bytes)\n';

        resultsData.forEach((data, index) => {
            if (!data) return;
            const finalHop = data.chain[data.chain.length - 1];
            const totalDuration = data.chain.reduce((sum, h) => sum + h.responseTime, 0);
            const size = finalHop.headers ? (finalHop.headers['content-length'] || '') : '';
        ...
        link.setAttribute('download', `tiinytools_bulk_http_status_${Date.now()}.csv`);
